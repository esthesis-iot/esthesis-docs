...
== Configuration
_esthesis__ comes with a plethora of configuration options to accommodate different hardware and
deployments as presented next. Mandatory parameters are
...
|fsRoot
|Alphanumeric
|The root of the filesystem where the platform can store data (for example, provisioning packages). If left empty, a folder named `.esthesis` under the user's home dir will be created. Note that in cluster configurations this filesystem needs to be shared among the nodes of the cluster.
