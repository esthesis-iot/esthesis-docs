...
Before you publish a Helm chart, ensure that the version of the Helm chart in `Chart.yaml` has been
updated.

To make the new Helm available, you need to push the the `esthesis-docs` repository.
